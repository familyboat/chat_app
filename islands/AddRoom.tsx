...
export default function AddRoom() {
  ...
  return (
    <form
      onSubmit={async (e) => {
        e.preventDefault();
        const create = Client.createRoom(roomName);
        try {
          const roomId = await create;
          location.pathname = "/" + roomId;
        } catch (err) {
          alert(`Cannot create room: ${(err as Error).message}`);
        }
      }}
    >
      <label>
      ...
      <button
        class="mt-7 flex items-center rounded-md h-8 py-2 px-4 bg-gray-800 font-medium text-sm text-white"
        type="submit"
      >
        create
      </button>
